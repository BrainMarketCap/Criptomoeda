...
class Blockchain:
    ...
    def getLatestBlock (self):
        ...
        def hashMatchesDifficulty (self, hash, difficulty):
            ...
            return hashBinary.starswith(requiredPrefix)

        def findBlock(self, index, previousHash, tomestamp, data, difficulty):
        nonce = 0
        while True:
        ...
        def getDifficulty(self):
            ...
            if latesdBlock.index ÷ self.DIFFICULTY_ADJUSTMENT == 0 and latestBlock.index != 0:
...
def calculateHash (index, previousHash, timestamp, data, difficult, nonce):
    return hashlib.sha256(str(index) + previousHash + str(timestamp) + data + difficulty + nonce) .hexdigest ()
